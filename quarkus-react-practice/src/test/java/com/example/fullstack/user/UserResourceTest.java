...
class UserResourceTest {
    ...
    void create() {
        given()
            .body(
                "{\"name\":\"test\",\"password\":\"test\",\"roles\":[\"user\"]}"
            )
    ...
    void createUnauthorized() {
        given()
            .body(
                "{\"name\":\"test-unauthorized\",\"password\":\"test\","
                    + "\"roles\":[\"user\"]}"
            )
            .contentType(ContentType.JSON)
            .when().post("/api/v1/users")
    ...
    void createDuplicate() {
        given()
            .body(
                "{\"name\":\"user\",\"password\":\"test\",\"roles\":[\"user\"]}"
            )
            .contentType(ContentType.JSON)
            .when().post("/api/v1/users")
            .then()
    ...
    void delete(UniAsserter asserter) {
        ...
        // The original code causes "No current vertx context found" error.
        // assertThat(
        //     User.findById(toDelete.id).await().indefinitely(),
        //     nullValue()
        // );
        ...
        asserter.assertEquals(() -> User.<User>findById(toDelete.id), null);
    }
    ...
    @TestSecurity(user = "admin", roles = "user")
    @TestReactiveTransaction
    void changePassword(UniAsserter asserter) {
        given()
            .body(
                "{\"currentPassword\": \"quarkus\", \"newPassword\": \"changed\"}"
            )
            .contentType(ContentType.JSON)
        ...
        asserter.assertEquals(
            () -> User.<User>findById(0L).map(
                s -> BcryptUtil.matches("changed", s.password)
            ),
            true
        );
    }
    ...
    void changePasswordDoesntMatch() {
        given()
            .body(
                "{\"currentPassword\": \"wrong\", \"newPassword\": \"changed\"}"
            )
            .contentType(ContentType.JSON)
            .when().put("/api/v1/users/self/password")
            .then()
